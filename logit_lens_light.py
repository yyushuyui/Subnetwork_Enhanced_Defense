...
def logit_lens_drawing(max_probs, words, input_tokens, result_dir, prompt_idx):
    """
    ...
    # Trim the columns to keep only the last column
    img = img[:, -1:]  # Keep only the last column
    words = [row[-1:] for row in words]  # Trim words to keep only the last column
    input_tokens = input_tokens[-1:]  # Trim input tokens to keep only the last colu
...
result_dir = "/cl/work10/shuyi-yu/LogitsEditingBasedDefense/results/logitlens_result1"
